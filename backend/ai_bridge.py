...
DEFAULT_TUTORIAL_ANSWER_CLASSIFIER_MODEL_DIR = Path(os.environ.get("TUTORIAL_ANSWER_CLASSIFIER_MODEL_DIR", str(ROOT / "runs" / "tutorial_answer_classifier_context_v9_clean")))
DEFAULT_TUTORIAL_PROGRESS_ANSWER_CLASSIFIER_MODEL_DIR = Path(os.environ.get("TUTORIAL_PROGRESS_ANSWER_CLASSIFIER_MODEL_DIR", str(ROOT / "runs" / "tutorial_answer_classifier_context_v9_clean")))
DEFAULT_TUTORIAL_STAGE_CLASSIFIER_MODEL_DIR = Path(os.environ.get("TUTORIAL_STAGE_CLASSIFIER_MODEL_DIR", str(ROOT / "runs" / "tutorial_stage_classifier_autonomy_v4_clean")))
...
def tutorial_answer_classifier_inference(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    if not checkpoint.exists() or not tokenizer.exists():
        return {
            "available": False,
            "reason": f"checkpoint not found: {checkpoint}",
            "expectedCommand": "python -m ai_models.tutorial_response.train_answer_classifier --device cuda --amp",
        }

    try:
        from ai_models.tutorial_response.infer_answer_classifier import TutorialAnswerClassifierInference
    except Exception as exc:  # pragma: no cover
        return {
            "available": False,
            "reason": f"TutorialAnswerClassifier import failed: {exc}",
            "expectedInstall": "pip install -r requirements-ml.txt",
        }
