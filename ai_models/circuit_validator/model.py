...
class CircuitValidatorTransformer(nn.Module):
    """Transformer encoder for neural circuit validation."""

    def __init__(self, config: CircuitValidatorConfig) -> None:
        super().__init__()
        self.config = config
        ...
        self.issue_head = nn.Linear(config.d_model, config.issue_count)
        self.risk_head = nn.Linear(config.d_model, config.risk_count)
        self.repair_head = nn.Linear(config.d_model, config.repair_count)
    ...
    def forward(self, input_ids: Tensor, attention_mask: Tensor) -> dict[str, Tensor]:
        ...
        return {
            ...
            "repair_logits": self.repair_head(pooled),
...
def load_checkpoint(checkpoint: str, map_location: str | torch.device = "cpu") -> tuple[CircuitValidatorTransformer, CircuitValidatorConfig, dict[str, Any]]:
    payload = torch.load(checkpoint, map_location=map_location)
    config = CircuitValidatorConfig.from_json(payload["config"])
    model = CircuitValidatorTransformer(config)
