...
from .config import CircuitValidatorConfig
from .data import CircuitDataset, collect_texts, collate_circuit_batch, generate_records, load_jsonl, write_jsonl
from .losses import accuracy, circuit_loss, multilabel_f1
from .model import CircuitValidatorTransformer, count_parameters
from .taxonomy import BOARD_CLASSES, CIRCUIT_ISSUES, REPAIR_ACTIONS, RISK_CLASSES


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train neural Circuit Validator.")
    parser.add_argument("--data", default="data/circuit_validator_training.jsonl")
    parser.add_argument("--output", default="runs/circuit_validator")
...
def main() -> None:
    ...
    config = CircuitValidatorConfig(
        vocab_size=len(tokenizer.vocab),
        issue_count=len(CIRCUIT_ISSUES),
    ...
    dataset = CircuitDataset(records, tokenizer, config.max_length)
    ...
    train_loader = DataLoader(train_dataset, batch_size=args.batch_size, shuffle=True, collate_fn=collate_circuit_batch)
    valid_loader = DataLoader(valid_dataset, batch_size=args.batch_size, shuffle=False, collate_fn=collate_circuit_batch)
    model = CircuitValidatorTransformer(config).to(device)
...
def run_epoch(
    ...
    model.train(train)
    total = {"loss": 0.0, "issue_f1": 0.0, "risk_acc": 0.0, "repair_acc": 0.0, "board_acc": 0.0}
    steps = 0
    for batch in loader:
        ...
        with torch.set_grad_enabled(train):
            with torch.cuda.amp.autocast(enabled=amp and device.type == "cuda"):
                ...
                loss, _parts = circuit_loss(outputs, batch)
            if train:
        ...
        total["issue_f1"] += multilabel_f1(outputs["issue_logits"].detach(), batch["issue_targets"])
        total["risk_acc"] += accuracy(outputs["risk_logits"].detach(), batch["risk_class"])
        total["repair_acc"] += accuracy(outputs["repair_logits"].detach(), batch["repair_class"])
...
def save_checkpoint(path: Path, model: CircuitValidatorTransformer, config: CircuitValidatorConfig, epoch: int, metrics: dict[str, float]) -> None:
    torch.save({"model_type": "circuit_validator_transformer", "model_state": model.state_dict(), "config": config.to_json(), "epoch": epoch, "metrics": metrics}, path)
